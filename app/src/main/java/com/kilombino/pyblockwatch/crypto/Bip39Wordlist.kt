...
/**
 * The BIP-39 English wordlist, verbatim and in order — the 2048 words whose index (0..2047)
 * is an 11-bit group of the mnemonic. Embedded rather than loaded from a resource so the same
 * bytes are used by the app and by the unit-test JVM, and so the whole seed path stays
 * auditable Kotlin. SHA-256 of the canonical one-word-per-line file:
...
""".trim().split(Regex("\\s+")))
    ...
    init { require(WORDS.size == 2048) { "BIP-39 wordlist must hold 2048 words, got ${WORDS.size}" } }

    val INDEX: Map<String, Int> = WORDS.withIndex().associate { (i, w) -> w to i }
}
